...
export function ReviewsSection({ reviews }: { reviews: ReviewItem[] }) {
  return (
    <div className="mt-10 border-t border-brand-border pt-6">
      <h2 className="text-lg font-semibold text-white">用户评价</h2>

      {reviews.length === 0 ? (
        <p className="mt-3 text-sm text-slate-400">暂无评价。</p>
      ) : (
        <div className="mt-4 space-y-4">
          {reviews.map((r, idx) => {
            ...
            return (
              <div key={`${r.created_at}-${idx}`} className="border border-brand-border bg-brand-surface rounded-lg p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full border border-white/10 overflow-hidden bg-slate-800 flex items-center justify-center text-xs text-slate-300">
                      {p?.avatar_url ? (
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => {
                          ...
                          return (
                            <Star key={i} className={active ? "h-4 w-4 fill-brand-action text-brand-action" : "h-4 w-4 text-slate-600"} />
                          );
                        })}
                      </div>
                    </div>
