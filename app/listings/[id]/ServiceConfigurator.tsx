...
export function ServiceConfigurator({
  ...
  return (
    <div className="border border-brand-border rounded-xl p-5 bg-brand-surface shadow-xl">
      {/* 套餐切换器 */}
      <div className="mb-6 flex p-1 bg-black/20 rounded-xl">
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                active
                  ? "bg-brand-surface text-brand-action shadow-lg shadow-black/20 scale-[1.02] border border-brand-border"
      ...
      <div className="flex items-baseline justify-between">
        <div className="text-3xl font-extrabold text-white" suppressHydrationWarning>
          ¥{formatMoney(basePrice)}
        </div>
      </div>

      {/* 交付 / 修改次数信息栏 */}
      <div className="mt-4 bg-black/40 rounded-lg p-3 flex justify-between items-center border border-brand-border" suppressHydrationWarning>
        <div className="flex items-center gap-2 text-sm text-slate-300">
          <Clock className="w-4 h-4 text-brand-action" />
          <span>{currentPackage.delivery_days} 天交付</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-slate-300">
          <RefreshCcw className="w-4 h-4 text-brand-action" />
          <span>
            {(() => {
      ...
      {options.length > 0 ? (
        <div className="mt-6 pt-6 border-t border-brand-border">
          <div className="text-sm font-bold text-white mb-4">增值选配</div>
              ...
              return (
                <label key={idx} className="flex items-start justify-between gap-3 cursor-pointer group">
                  <span className="flex items-start gap-3">
                    <input
                      type="checkbox"
                      className="mt-1 h-4 w-4 rounded border-brand-border bg-black/20 text-brand-action focus:ring-brand-action/30"
                      checked={checked}
                      onChange={(e) => {
                  ...
                  <span className="text-sm font-semibold text-slate-200">+¥{formatMoney(opt.price)}</span>
                </label>
              );
            })}
          </div>
      ...
      <div className="mt-8 pt-6 border-t border-brand-border flex items-center justify-between">
        <div>
          ...
          <div className="text-2xl font-extrabold text-brand-action">¥{formatMoney(total)}</div>
        </div>

        <button
          type="button"
          className="rounded-xl bg-brand-action hover:bg-amber-600 active:scale-95 transition-all text-white px-6 py-3 font-bold shadow-lg shadow-black/20"
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-xl border border-brand-border bg-brand-surface p-6 shadow-2xl">
            <div className="text-xl font-extrabold text-white">填写下单需求</div>
            <div className="mt-2 text-sm text-slate-400">
              已选 <span className="font-bold text-slate-200">{TIER_LABELS[selectedTier].subtitle}</span>
            ...
            {error ? (
              <div className="mt-4 rounded-xl border border-red-900/50 bg-red-900/20 px-4 py-3 text-sm text-red-400 font-medium">
                {error}
              </div>
            ) : null}
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button
                type="button"
                className="rounded-xl border border-brand-border px-6 py-2.5 text-sm font-bold text-slate-400 hover:bg-white/5 transition-colors"
                disabled={pending}
              ...
              <button
                ...
                className="rounded-xl bg-brand-action text-white px-8 py-2.5 text-sm font-bold shadow-lg shadow-black/20 hover:bg-amber-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"
              >
                {pending ? "提交中..." : "确认下单"}
              </button>
            </div>
